        ...
        with tracer.start_as_current_span("LoadTextNode") as span:
            ...
            try:
                if state.chapter_text:
                    ...
                    node_outputs["text_summary"] = str(
                        state.chapter_text[:200] + "..."
                        if state.chapter_text and len(state.chapter_text) > 200
                        else state.chapter_text or ""
                    )
    ...
    async def service(self, store: ChapterParsingStore) -> None:
        with tracer.start_as_current_span("ChunkTextNode") as span:
            ...
            if state.chapter_text:
                ...
                node_inputs["chapter_text_summary"] = str(
                    state.chapter_text[:200] + "..."
                    if len(state.chapter_text) > 200
                    else state.chapter_text
                )
            span.set_attribute("braintrust.input", json.dumps(node_inputs))
            ...
            try:
                ...
                if chunks:
                    node_outputs["first_chunk_summary"] = str(
                        chunks[0][:200] + "..." if len(chunks[0]) > 200 else chunks[0]
                    )
...
class PromptConstructionNode(Node[ChapterParsingStore]):
    ...
    async def service(self, store: ChapterParsingStore) -> None:
        with tracer.start_as_current_span("PromptConstructionNode") as span:
            ...
            if state.chunks:
                node_inputs["first_chunk_summary"] = str(
                    state.chunks[0][:200] + "..."
                    if state.chunks[0] and len(state.chunks[0]) > 200
                    else state.chunks[0] or ""
                )
            span.set_attribute("braintrust.input", json.dumps(node_inputs))
...
"""
                ...
                if prompts:
                    node_outputs["first_prompt_summary"] = str(
                        prompts[0][:200] + "..."
                        if prompts[0] and len(prompts[0]) > 200
                        else prompts[0] or ""
                    )
...
class GeminiApiNode(Node[ChapterParsingStore]):
    ...
    async def service(self, store: ChapterParsingStore) -> None:
        with tracer.start_as_current_span("GeminiApiNode") as span:
            ...
            if state.prompts:
                node_inputs["first_prompt_summary"] = str(
                    state.prompts[0][:200] + "..."
                    if state.prompts[0] and len(state.prompts[0]) > 200
                    else state.prompts[0] or ""
                )
            span.set_attribute("braintrust.input", json.dumps(node_inputs))
            ...
            try:
                ...
                if raw_responses:
                    node_outputs["first_response_summary"] = str(
                        str(raw_responses[0])[:200] + "..."
                        if raw_responses[0] and len(str(raw_responses[0])) > 200
                        else str(raw_responses[0]) or ""
                    )
                ...
                span.add_event(
                    "state_transition",
                    {"set": "raw_responses", "num_responses": len(raw_responses)},
                )
                span.set_attribute("braintrust.output", json.dumps(node_outputs))
    ...
    async def service(self, store: ChapterParsingStore) -> None:
        with tracer.start_as_current_span("ParseSegmentsNode") as span:
            ...
            if state.raw_responses:
                node_inputs["first_raw_response_summary"] = str(
                    str(state.raw_responses[0])[:200] + "..."
                    if state.raw_responses[0] and len(str(state.raw_responses[0])) > 200
                    else str(state.raw_responses[0]) or ""
                )
            span.set_attribute("braintrust.input", json.dumps(node_inputs))
            ...
            all_segments: list[
                TextSegment
            ] = []  # Define all_segments at a scope visible to the finally/except block if needed for output
            try:
                if not state.raw_responses or not state.prompts:
                    raise ValueError(
                        "raw_responses and prompts must be set before parsing segments."
                    )
                ...
                for idx, (raw, prompt) in enumerate(
                    zip(state.raw_responses, state.prompts, strict=False)
                ):
                    for attempt in range(3):  # Max 3 attempts
                        try:
                            response_text = (
                                raw.strip()
                                if attempt == 0
                                else await call_gemini_api(prompt, model_name, api_key)
                            )
                            match = re.search(
                            ...
                            if isinstance(data, dict) and "segments" in data:
                                ...
                                raise ValueError(
                                    f"Expected a list or dict with 'segments', got: {type(data)}"
                                )

                            if not isinstance(segments_data, list):
                                raise ValueError(
                                    f"Expected a list of segments, got: {type(segments_data)}"
                                )
                            ...
                            for seg_dict in segments_data:
                                ...
                                if (
                                    ...
                                    and speaker.strip().lower() == "narrator"
                                ):
                            ...
                            if attempt == 2:  # Last attempt failed
                                span.add_event(
                                    "parse_chunk_error_final",
                                    {
                                        "chunk_idx": idx,
                                        "error": str(e_attempt),
                                        "raw_response_snippet": raw[:100],
                                    },
                                )
                ...
                if all_segments:
                    try:
                        ...
                        node_outputs["first_segment_summary"] = str(
                            first_segment_json[:200] + "..."
                            if len(first_segment_json) > 200
                            else first_segment_json
                        )
                ...
                # Log state transition
                span.add_event(
                    "state_transition", {"set": "segments", "num_segments": len(all_segments)}
                )
                span.set_attribute("braintrust.output", json.dumps(node_outputs))
                ...
                node_outputs["error"] = error_message
                node_outputs["num_segments_before_error"] = len(
                    all_segments
                )  # Log how many were processed
                span.set_attribute("error", error_message)
                await store.set_state(
                    {"error": error_message, "segments": all_segments}
                )  # Save partial if any
                # Log error state transition
                span.add_event("state_transition", {"set": "error", "value": error_message})
                span.set_attribute("braintrust.output", json.dumps(node_outputs))
        ...
        with tracer.start_as_current_span("MergeSegmentsNode") as span:
            ...
            if state.segments:
                try:
                    ...
                    node_inputs["first_segment_summary"] = str(
                        first_segment_json[:200] + "..."
                        if len(first_segment_json) > 200
                        else first_segment_json
                    )
            ...
            try:
                if not state.segments:  # Check if segments list is None or empty
                    raise ValueError(
                        "segments must be set and non-empty before merging into Chapter."
                    )
                if len(state.segments) == 0:  # Explicit check for empty list
                    raise ValueError(
                        "No segments produced: at least one segment is required to build a Chapter."
                    )
...
class ErrorHandlingNode(Node[ChapterParsingStore]):
    ...
    async def service(self, store: ChapterParsingStore) -> None:
        with tracer.start_as_current_span("ErrorHandlingNode") as span:
            ...
            node_inputs: dict[str, Any] = {
                "error_message_from_state": str(state.error or "No error message present in state.")
            }
            span.set_attribute("braintrust.input", json.dumps(node_inputs))

            node_outputs: dict[str, Any] = {
                "processed_error": str(state.error or "") if state.error else None
            }
            if state.error:
    ...
    async def service(self, store: ChapterParsingStore) -> None:
        with tracer.start_as_current_span("SaveResultsNode") as span:
            ...
            try:
                ...
                if state.chapter_text:
                    (chapter_data_path / "text.txt").write_text(
                        state.chapter_text, encoding="utf-8"
                    )
                ...
                if (
                    state.character_catalogue
                ):  # This was not in the original inputs to log, adding output logic
                    with (chapter_data_path / "characters.json").open("w", encoding="utf-8") as f:
                        json.dump(
                            state.character_catalogue.model_dump(), f, indent=2, ensure_ascii=False
                        )
                    files_saved.append("characters.json")

                node_outputs["files_saved"] = (
                    files_saved if files_saved else []
                )  # Ensure it's a list
                # Log state transition
...
try:
    workflow = Workflow(name="Chapter Parsing Pipeline (Junjo-Native)", graph=graph)
...
class ChapterParsingWorkflow:
        ...
        if self.workflow:
            try:
                ...
                result = {
                    ...
                    "segments": [seg.dict() for seg in final_state.segments]
                    if final_state.segments
                    else [],
                    "characters": final_state.character_catalogue.dict()
                    if final_state.character_catalogue
                    else {},
    ...
    async def _simple_parse(
        self, text_content: str, progress_callback: Callable[[float], Awaitable[None]] | None = None
    ) -> dict[str, Any]:
        ...
        chapter = Chapter(chapter_number=1, title="Chapter 1", segments=[segment])
