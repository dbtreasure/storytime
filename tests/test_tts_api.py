...
def test_assign_and_get_voice():
    ...
    resp = client.post("/api/v1/characters/TestChar/voice", json={"provider": "openai", "voice_id": voice_id})
    assert resp.status_code == 200
    # Get
...
def test_preview_voice(tmp_path):
    ...
    assert openai_voices, "No OpenAI voices found"
    voice_id = openai_voices[0]["id"]
    resp = client.post("/api/v1/voices/preview", json={"provider": "openai", "voice_id": voice_id, "text": "Hello world"})
    assert resp.status_code == 200
...
def test_assign_voice_nonexistent_character():
    resp = client.post("/api/v1/characters/NoSuchChar/voice", json={"provider": "openai", "voice_id": "alloy"})
    assert resp.status_code == 404
...
def test_preview_voice_invalid_provider():
    resp = client.post("/api/v1/voices/preview", json={"provider": "notarealprovider", "voice_id": "alloy", "text": "Hello world"})
...
def test_generate_tts_unsupported_provider():
    resp = client.post("/api/v1/tts/generate", json={"chapter_text": "Hello world!", "provider": "notarealprovider"})
    assert resp.status_code == 400
