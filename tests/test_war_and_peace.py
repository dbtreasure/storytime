    ...
    try:
        ...
        for i, segment in enumerate(chapter.segments[:5], 1):
            speaker_type = (
                ...
                if isinstance(segment.speaker_type, str)
                else segment.speaker_type.value
            )
            print(
                f"   {i}. [{speaker_type.upper()}]: {segment.text[:80]}{'...' if len(segment.text) > 80 else ''}"
            )
            if segment.instruction:
                print(
                    f"      🎙️  Instruction: {segment.instruction[:100]}{'...' if len(segment.instruction) > 100 else ''}"
                )
        ...
        print("   Model: gpt-4o-mini-tts (with instructions support)")
        ...
        print(f"   Characters: {', '.join(sorted(chapter.get_unique_characters()))}")
        print("   Narrator voice: echo")
        print("   Character voices: Dynamic assignment based on gender/age hints")
        print("   Professional audiobook format with dialogue separation ✅")
        print("   Enhanced TTS instructions for optimal delivery ✅")
